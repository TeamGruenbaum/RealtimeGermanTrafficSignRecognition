...
def __detect(detector, request_id):
    if flask.request.files.get('image') is None:
        return flask.jsonify(ErrorResponse('No image provided').serialize()), 400
    else:
        converted = cv2.imdecode(numpy.frombuffer(flask.request.files.get('image').stream.read(), numpy.uint8), -1)
        image_detection_results = detector.detect(converted)

        return flask.jsonify({
                    "requestId": request_id,
                    "imageDetectionResults": [image_detection_result.serialize() for image_detection_result in image_detection_results]
               }), 200


@app.route('/machine-learning-image-analysis/image-detection-results', methods=['POST'])
def detect_with_machine_learning_image_analysis():
    return __detect(machine_learning_image_analysis_detector, flask.request.args.get('requestId'))


@app.route('/traditional-image-analysis/image-detection-results', methods=['POST'])
def detect_with_traditional_image_analysis():
    return __detect(traditional_image_analysis_detector, flask.request.args.get('requestId'))

@app.errorhandler(Exception)
def handle_exception(exception):
    print(exception)
    return flask.jsonify(ErrorResponse("Some unexpected error occurred.").serialize()), 500



if __name__ == '__main__':
    app.run(host="0.0.0.0", port=15240, debug=False)
